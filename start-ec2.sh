...
instance_state=$(aws ec2 describe-instance-status --region us-east-1 --instance-id $instance_id --query 'InstanceStatuses[*].InstanceState.Name' --output text)
size=${#instance_state}
...
instance_start_invoke=$(aws ec2 start-instances --region us-east-1 --instance-ids $instance_id --query 'StartingInstances[*].CurrentState.Name' --output text)
...
do
fetch_instance_start=$(aws ec2 start-instances --region us-east-1 --instance-ids $instance_id --query 'StartingInstances[*].CurrentState.Name' --output text)
...
do
fetch_instance_health=$(aws ec2 describe-instance-status --region us-east-1 --instance-id $instance_id --query 'InstanceStatuses[*].InstanceStatus.Status' --output text)
...
instance_ip=$(aws ec2 describe-instances --region us-east-1 --instance-id $instance_id --query "Reservations[*].Instances[*].PublicIpAddress" --output=text)
fi
fi
